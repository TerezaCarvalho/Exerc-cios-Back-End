// Utilizando o operador $all, retorne todos os filmes que contenham action e adventure no
// array category.
db.movies.find({category: {$all: ["action", "adventure"]}});
// dentro de category TEM QUE TER action e adventure ao mesmo tempo

// Agora retorne os filmes que contenham action no array category e possuem nota
// do IMDB maior do que 7.
...
db.movies.find({ratings: {$elemMatch: {$gt: 103}}}, {title: 1, ratings: 1}).pretty();

// Retorne todos os filmes com ratings entre 100 e 105, exibindo apenas os campos title e ratings.
db.movies.find({ratings: {$elemMatch: {$gte: 100, $lte: 105}}}, {title: 1, ratings: 1}).pretty();

// Retorne todos os filmes com ratings entre 64 e 105 e que é divisível por 9,
// exibindo apenas os campos title e ratings.
db.movies.find({ratings: {$elemMatch: {$gte: 64, $lte: 105, $mod:[9,0]}},}, {title: 1, ratings: 1}).pretty();
...
db.movies.find({category: {$all: ["adventure"]}, ratings:{$elemMatch: {$gt: 103}}}, {title:1, ratings: 1, category: 1}).pretty();

// Retorne somente o título de todos os filmes com dois elementos no array category.
db.movies.find({category: {$size: 2}}, {title: 1, category: 1}).pretty();

// Retorne somente o título de todos os filmes com quatro elementos no array ratings.
db.movies.find({ratings: {$size: 4}}, {title: 1, ratings: 1}).pretty();

// Busque os filmes em que o módulo 5 do campo budget seja 0 e que o array category tenha tamanho 2.
db.movies.updateOne({title: "Batman"}, {$set:  {budget: 35}});
db.movies.updateOne({title: "Godzilla"}, {$set:  {budget: 10}});
db.movies.updateOne({title: "Home Alone"}, {$set:  {budget: 7.4}});
db.movies.find({category: {$size: 4}, budget: {$mod: [5, 0]}}).pretty();

// Retorne os filmes da categoria "sci-fi" ou que possua o ratings maior do que 199,
// exibindo apenas os campos title, ratings e category.
db.movies.find({$or: [
  {cattegory: {$all: ["sci-fi"]}}, {ratings: {$elemMatch: {$gt:199}}}
]}, {title: 1, ratings: 1, category: 1}).pretty();
...
db.movies.find({$and: [ {$or: [ {ratings: {$size: 4}}, {category: {$all: ["adventure", "family"]}} ]}, {imdbRating: {$gt: 7}}]}).pretty();
